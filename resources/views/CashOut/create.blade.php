    ...
    <div class="max-w-4xl mx-auto mt-10 bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div class="p-4 bg-orange-100 rounded-lg text-center shadow">
                ...
                <h3 id="initial-cash" class="text-lg font-semibold text-gray-800">${{ number_format($initialCash ?? 0, 2) }}</h3>
            </div>
            ...
            <div class="p-4 bg-green-100 rounded-lg text-center shadow">
                ...
                <h3 id="remaining-cash" class="text-lg font-semibold text-gray-800">${{ number_format($initialCash ?? 0, 2) }}</h3>
            </div>
        </div>
        ...
        <form action="{{ route('cashouts.store') }}" method="POST" id="cashout-form">
            ...
            <div class="mt-6 flex justify-end">
                <button type="submit" class="orange-btn">Enregistrer</button>
            </div>
        </form>
    </div>

    <script>
        let initialCash = parseFloat('{{ $initialCash ?? 0 }}');
        const minCash = 50;
